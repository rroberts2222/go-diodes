...
var _ = Describe("ManyToOne", func() {
	...
	Context("test", func() {
		It("drops only a few, not the whole buffer", func() {
			spy := newSpyAlerter()
			d := diodes.NewManyToOne(3, spy)
			...
			dataOverflow := []byte("last-data")
			d.Set(diodes.GenericDataType(&dataOverflow))
			data, ok := d.TryNext()
			Expect(ok).To(BeTrue())
			dataBytes := (*[]byte)(data)
			Expect(string(*dataBytes)).To(Equal("some-data-1"))
			data, ok = d.TryNext()
			Expect(ok).To(BeTrue())
			dataBytes = (*[]byte)(data)
			Expect(string(*dataBytes)).To(Equal("some-data-2"))
			data, ok = d.TryNext()
			Expect(ok).To(BeTrue())
			dataBytes = (*[]byte)(data)
			Expect(string(*dataBytes)).To(Equal("last-data"))
			Expect(spy.AlertInput.Missed).To(Receive(Equal(1)))
		})
		FIt("drops only a few, not the whole buffer", func() {
			spy := newSpyAlerter()
			d := diodes.NewOneToOne(3, spy)
